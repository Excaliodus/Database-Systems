...
--1
Select	MAX(u.Age) as MaxAge, MIN(u.Age) as MinAge,  STDEV(u.Age) as StdevAge
from Users as u


--2
Select  u.UserName,count(*) as NoOfFollowers
from Users as u,Following as F
...
--3
Select  table1.FollowedUserName,table1.temp
from (select FollowedUserName,count(*) as temp
from Following group by FollowedUserName) as table1
where 1=(select count(distinct temp)
         from (select FollowedUserName,count(*) as temp from Following group by FollowedUserName)as table2
		 where table2.temp>table1.temp)

--4
Select  u.UserName
from Users as u,Tweets as t
Except(Select  t.UserName
from Tweets as t)


--5
select H.Hashtag,T.UserName,count(*) as noOfTimes
...
--9(i)
select i.Description, count(*) MostCommonInterest
...
group by i.Description)
--9(ii)
select i.Description, count(*) LeastCommonInterest
from Interests as i, UserInterests as u
where i.InterestID=u.InterestID
group by i.Description
having count(*)<=ALL(select count(*)
...
--10
select u.Country,count(*) as totalNoOftweets
from users as u, Tweets as t
...
from Tweets as t
group by t.UserName
having count(*)>(select AvG(t1.temp)
from (select count(*)as temp
from Tweets
group by Tweets.UserName)as t1)


--12
Select  u.UserName
from Users as u,Following as F, Users as u2
where u.UserName=F.FollowedUserName and f.FollowerUserName=u2.UserName and u2.Country='Pakistan'
Group by u.UserName
having count(*)>=1

--13
select i.InterestID, i.Description, count(*)
from Interests as i, UserInterests as u
where i.InterestID=u.InterestID
